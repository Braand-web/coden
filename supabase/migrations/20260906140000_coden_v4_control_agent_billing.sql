...
create policy billing_provider_customers_owner_select on public.billing_provider_customers for select to authenticated using (exists (select 1 from public.billing_accounts a where a.id = account_id and a.owner_user_id = (select auth.uid())));
create policy billing_subscriptions_owner_select on public.billing_subscriptions_v2 for select to authenticated using (exists (select 1 from public.billing_accounts a where a.id = account_id and a.owner_user_id = (select auth.uid())));
create policy credit_grants_owner_select on public.credit_grants for select to authenticated using (exists (select 1 from public.billing_accounts a where a.id = account_id and a.owner_user_id = (select auth.uid())));
create policy usage_events_owner_select on public.usage_events for select to authenticated using (exists (select 1 from public.billing_accounts a where a.id = account_id and a.owner_user_id = (select auth.uid())));
create policy usage_reservations_owner_select on public.usage_reservations for select to authenticated using (exists (select 1 from public.billing_accounts a where a.id = account_id and a.owner_user_id = (select auth.uid())));
create policy usage_settlements_owner_select on public.usage_settlements for select to authenticated using (exists (select 1 from public.usage_reservations r join public.billing_accounts a on a.id = r.account_id where r.id = reservation_id and a.owner_user_id = (select auth.uid())));
create policy credit_ledger_owner_select on public.credit_ledger_entries for select to authenticated using (exists (select 1 from public.billing_accounts a where a.id = account_id and a.owner_user_id = (select auth.uid())));
create policy auto_topup_owner_select on public.auto_topup_configs for select to authenticated using (exists (select 1 from public.billing_accounts a where a.id = account_id and a.owner_user_id = (select auth.uid())));
create policy spend_limits_owner_select on public.spend_limits for select to authenticated using (exists (select 1 from public.billing_accounts a where a.id = account_id and a.owner_user_id = (select auth.uid())));
create policy billing_alerts_owner_select on public.billing_alerts_v2 for select to authenticated using (exists (select 1 from public.billing_accounts a where a.id = account_id and a.owner_user_id = (select auth.uid())));
