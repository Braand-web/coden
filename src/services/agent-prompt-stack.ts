...
const CODEN_CLOUD_POLICY = [
  ...
  'Use Coden Cloud by default. Coden Cloud is the managed backend layer that creates or tracks backend namespace/project, SQL schema, RLS, auth settings, storage buckets, functions, secrets, public runtime config, logs, and usage.',
  ...
  'For fullstack apps, generate a real backend contract alongside the frontend: src/lib/codenCloud.ts or equivalent browser-safe client, src/lib/appData.ts or equivalent CRUD layer, supabase/schema.sql with RLS policies and explicit Data API grants, and a smoke test. Do not return only a static preview.',
...
const CODEN_PRODUCTION_READINESS_POLICY = [
  ...
  'A real data app needs a real backend contract: Supabase/Coden Cloud schema, RLS, policies, browser-safe client, validation, and runner checks. Do not rely on localStorage for production persistence.',
...
const CODEN_GENERATION_PRODUCT_POLICY = [
  ...
  'Select exactly one Coden runtime profile. Use tanstack-fullstack for real applications with backend data, authentication, storage, realtime, payments, server functions, SSR, SEO, or publication. Use vite-static only for genuinely static apps without a backend.',
  ...
  'For new apps, return a complete profile-specific project including package.json, the correct Vite/TanStack configuration, tsconfig.json, index.html or route entry, src/, README.md, tests, wrangler.jsonc for Cloudflare Workers profiles, and coden/app-manifest.json.',
...
const CODEN_ZERO_BUG_GENERATION_POLICY = [
  ...
  'Every new app must include complete profile-specific files: package.json, configuration, source entry/routes, tests, README.md, and coden/app-manifest.json. tanstack-fullstack additionally includes src/server.ts, src/routes/, src/lib/query-client.ts, and wrangler.jsonc.',
