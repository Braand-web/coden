...
const DB_NAME = 'coden-prompt-attachments';
const DB_VERSION = 1;
const STORE_NAME = 'attachments';
const FALLBACK_KEY = 'coden-pending-attachment-names';
const MAX_FILES = 6;
...
function notify(message: string, kind: 'info' | 'success' | 'error' = 'info') {
  const existing = document.getElementById('coden-prompt-toast');
  existing?.remove();

  const toast = document.createElement('div');
  toast.id = 'coden-prompt-toast';
  ...
  toast.style.cssText = [
    ...
    'border:1px solid var(--border,color-mix(in srgb, var(--border) 16%, transparent))',
    'border-radius:12px',
    'padding:10px 12px',
    'font:12px/1.45 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',
    'color:var(--foreground)',
    kind === 'error' ? 'background:var(--danger-background)' : kind === 'success' ? 'background:var(--success-background)' : 'background:var(--surface)',
    'box-shadow:0 18px 60px color-mix(in srgb, var(--foreground) 16%, transparent)',
  ].join(';');
...
function ensureStyles() {
  if (document.getElementById('coden-prompt-actions-style')) return;
  const style = document.createElement('style');
  style.id = 'coden-prompt-actions-style';
  style.textContent = `
    ...
    .prompt-attachment-chip {
      ...
      background: color-mix(in srgb, var(--input) 94%, var(--foreground) 6%);
      color: var(--foreground);
      border-radius: 12px;
      padding: 6px 7px;
      font-size: 11px;
    ...
    .prompt-attachment-status {
      ...
      border-radius: 999px;
      flex: 0 0 auto;
      background: var(--text-secondary, var(--border-strong));
    }
    .prompt-attachment-status.is-uploaded { background: var(--success); }
    .prompt-attachment-status.is-failed { background: var(--danger); }
    .prompt-attachment-status.is-pending { background: var(--syntax-orange); }
