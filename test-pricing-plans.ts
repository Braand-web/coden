...
assert.equal(priceFor('business', 100, 'monthly').amountUsd, 50, 'Business 100 monthly must preserve its original public price.');
...
assert(catalog.prices.some(price => price.plan === 'pro' && price.credits === 100 && price.interval === 'monthly' && price.amount === 15_000), 'The original Pro 100 tier must remain 15 000 XAF.');
assert(catalog.plans.find(plan => plan.key === 'pro')?.tiers.includes(10_000), 'Pro must retain every historical tier through 10 000 credits.');
assert(catalog.plans.find(plan => plan.key === 'business')?.tiers.includes(10_000), 'Business must retain every historical tier through 10 000 credits.');
assert(catalog.prices.some(price => price.plan === 'business' && price.credits === 100 && price.interval === 'monthly' && price.amount === 30_000), 'Business must preserve 100 credits for 30 000 XAF.');
