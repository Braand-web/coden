...
const CONNECTORS: ConnectorDefinition[] = [
  { id: 'coden-cloud', name: 'Coden Cloud', description: 'Built-in backend, database, auth and storage.', category: 'Developer', initials: 'HC', color: '#2f6df6', builtIn: true, capabilities: ['Database and authentication', 'Storage and realtime', 'Protected server functions'] },
  { id: 'coden-ai', name: 'Coden AI', description: 'Secure AI features for generated applications.', category: 'Developer', initials: 'AI', color: '#8b5cf6', builtIn: true, capabilities: ['Server-side AI connector', 'Streaming responses', 'Usage and secret protection'] },
  ...
  { id: 'saspay', name: 'Saspay', description: 'Paiements Coden sécurisés via le compte FlowPay.', category: 'Sales', initials: 'SP', color: '#2f6df6', builtIn: true, capabilities: ['Checkout en FCFA', 'Webhooks signés', 'Crédits après paiement confirmé'] },
...
function installStyle() {
  ...
  style.textContent = `
    ...
    .connectors-panel{width:min(1180px,calc(100vw - 40px));height:min(760px,calc(100dvh - 40px));display:grid;grid-template-columns:minmax(210px,260px) minmax(0,1fr);overflow:hidden;border:1px solid var(--border-mid,var(--border,var(--horizon-border)));border-radius:18px;background:var(--bg-surface,var(--horizon-surface));color:var(--text,var(--horizon-text));box-shadow:0 34px 110px rgba(0,0,0,.54);transform:translateY(12px) scale(.988);transition:transform 220ms cubic-bezier(.22,1,.36,1)}
    .connectors-overlay.open .connectors-panel{transform:translateY(0) scale(1)}
    .connectors-sidebar{min-height:0;display:flex;flex-direction:column;gap:12px;padding:14px;border-right:1px solid var(--border-light,color-mix(in srgb,var(--text,var(--horizon-border)) 9%,transparent));background:color-mix(in srgb,var(--bg,var(--horizon-surface)) 94%,transparent)}
    .connectors-search{height:42px;display:flex;align-items:center;gap:9px;padding:0 12px;border:1px solid var(--border,var(--horizon-border));border-radius:11px;background:var(--bg-input,var(--horizon-surface));color:var(--text-sub,var(--horizon-muted))}
    .connectors-search:focus-within{border-color:var(--border-focus,var(--horizon-blue))}
    .connectors-search svg{width:18px;height:18px;flex:none}
    .connectors-search input{width:100%;border:0;outline:0;background:transparent;color:var(--text,var(--horizon-text));font:inherit;font-size:14px}
    .connectors-nav{min-height:0;flex:1;display:flex;flex-direction:column;gap:4px;overflow:auto;padding-right:2px}
    .connectors-nav-label{padding:10px 10px 5px;color:var(--text-sub,var(--horizon-muted));font-size:11px;font-weight:720}
    .connectors-filter{width:100%;min-height:38px;display:flex;align-items:center;justify-content:space-between;gap:10px;border:0;border-radius:10px;padding:0 10px;background:transparent;color:var(--text-muted,var(--horizon-muted));font:inherit;font-size:13px;font-weight:650;text-align:left;cursor:pointer}
    .connectors-filter:hover,.connectors-filter.active{background:var(--bg-elevated,var(--horizon-raised));color:var(--text,var(--horizon-text))}
    .connectors-filter span:last-child{color:var(--text-sub,var(--horizon-muted));font-size:12px;font-variant-numeric:tabular-nums}
    .connectors-sidebar-card{display:grid;gap:9px;padding:13px;border:1px solid var(--border,var(--horizon-border));border-radius:12px;background:var(--bg-input,var(--horizon-surface))}
    .connectors-sidebar-card svg{width:19px;height:19px}
    .connectors-sidebar-card strong{font-size:13px}
    .connectors-sidebar-card p{margin:0;color:var(--text-sub,var(--horizon-muted));font-size:11px;line-height:1.45}
    .connectors-sidebar-card button,.connectors-secondary{height:36px;border:1px solid var(--border,var(--horizon-border));border-radius:10px;background:transparent;color:var(--text,var(--horizon-text));font:inherit;font-size:12px;font-weight:760;cursor:pointer}
    .connectors-main{min-width:0;min-height:0;display:grid;grid-template-rows:auto minmax(0,1fr);background:var(--bg-surface,var(--horizon-surface))}
    .connectors-header{min-height:58px;display:flex;align-items:center;justify-content:space-between;padding:0 20px;border-bottom:1px solid var(--border-light,color-mix(in srgb,var(--text,var(--horizon-border)) 9%,transparent))}
    ...
    .connectors-close{width:38px;height:38px;display:grid;place-items:center;border:0;border-radius:10px;background:transparent;color:var(--text-sub,var(--horizon-muted));cursor:pointer}.connectors-close:hover{background:var(--bg-elevated,var(--horizon-raised));color:var(--text,var(--horizon-text))}.connectors-close svg{width:21px;height:21px}
    .connectors-body{position:relative;min-height:0;overflow:auto;scrollbar-width:thin}
    .connectors-intro{position:relative;min-height:280px;display:grid;place-items:center;overflow:hidden;border-bottom:1px solid var(--border-light,color-mix(in srgb,var(--text,var(--horizon-border)) 7%,transparent))}
    ...
    .connector-chip{height:34px;display:flex;align-items:center;gap:8px;padding:0 11px;border:1px solid color-mix(in srgb,var(--text,var(--horizon-border)) 8%,transparent);border-radius:999px;background:rgba(25,25,25,.56);color:var(--horizon-text);font-size:12px;font-weight:720;white-space:nowrap}
    .connectors-intro::after{content:"";position:absolute;inset:0;background:linear-gradient(to bottom,rgba(25,25,25,.12),rgba(25,25,25,.68) 52%,var(--bg-surface,var(--horizon-surface)) 92%);pointer-events:none}
    ...
    .connectors-intro-copy h2{margin:0;color:var(--text,var(--horizon-text));font-size:clamp(24px,3vw,36px);line-height:1.04;letter-spacing:-.04em}
    .connectors-intro-copy p{max-width:570px;margin:0;color:var(--text-muted,var(--horizon-muted));font-size:13px;line-height:1.5}
    .connectors-intro-actions{display:flex;gap:9px}.connectors-intro-actions button,.connectors-intro-actions a{height:38px;display:inline-flex;align-items:center;justify-content:center;border:1px solid var(--border,var(--horizon-border));border-radius:10px;padding:0 14px;background:var(--bg-input,var(--horizon-raised));color:var(--text,var(--horizon-text));font-size:12px;font-weight:760;text-decoration:none;cursor:pointer}
    .connectors-catalog{padding:26px 30px 38px}.connectors-catalog-head{display:flex;align-items:end;justify-content:space-between;gap:16px;margin-bottom:16px}.connectors-catalog-head h3{margin:0 0 4px;font-size:18px}.connectors-catalog-head p{margin:0;color:var(--text-sub,var(--horizon-muted));font-size:12px}.connectors-count{color:var(--text-sub,var(--horizon-muted));font-size:12px}
    .connectors-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}
    .connector-card{min-width:0;display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:10px;border:1px solid var(--border,var(--horizon-border));border-radius:12px;padding:10px;background:var(--bg-input,var(--horizon-raised));color:var(--text,var(--horizon-text));text-align:left;cursor:pointer;transition:transform 150ms cubic-bezier(.22,1,.36,1),border-color 150ms,background 150ms}
    .connector-card:hover{transform:translateY(-1px);border-color:var(--border-focus,var(--horizon-blue));background:var(--bg-elevated,var(--horizon-raised))}
    .connector-card-copy{min-width:0}.connector-card-copy strong{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:14px}.connector-card-copy span{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-top:3px;color:var(--text-sub,var(--horizon-muted));font-size:11px}
    .connector-glyph{width:40px;height:40px;display:grid;place-items:center;flex:none;border-radius:10px;background:var(--connector-color);color:var(--horizon-text);font-size:11px;font-weight:900;letter-spacing:-.03em;box-shadow:0 0 0 1px color-mix(in srgb,var(--text,#111) 9%,transparent) inset}.connector-glyph.compact{width:25px;height:25px;border-radius:999px;font-size:8px}
    .connector-status{min-height:25px;display:inline-flex;align-items:center;border-radius:8px;padding:0 8px;background:var(--accent-dim,rgba(47,109,246,.14));color:var(--accent,var(--horizon-blue));font-size:10px;font-weight:820;white-space:nowrap}.connector-status.enabled{background:rgba(73,151,48,.22);color:var(--horizon-success)}.connector-status.setup_required{background:rgba(230,158,44,.18);color:var(--horizon-warning)}
    .connectors-empty{grid-column:1/-1;padding:50px 20px;color:var(--text-sub,var(--horizon-muted));text-align:center}
    .connector-detail{position:absolute;inset:0;z-index:5;display:grid;grid-template-rows:auto minmax(0,1fr);background:color-mix(in srgb,var(--bg-surface,var(--horizon-surface)) 97%,transparent);backdrop-filter:blur(18px);transform:translateX(102%);transition:transform 220ms cubic-bezier(.22,1,.36,1)}.connector-detail.open{transform:translateX(0)}
    .connector-detail-head{min-height:68px;display:flex;align-items:center;gap:12px;padding:0 24px;border-bottom:1px solid var(--border-light,color-mix(in srgb,var(--text,var(--horizon-border)) 8%,transparent))}.connector-detail-back{width:36px;height:36px;border:0;border-radius:9px;background:var(--bg-input,var(--horizon-raised));color:var(--text,var(--horizon-text));cursor:pointer}.connector-detail-head strong{font-size:15px}
    .connector-detail-body{overflow:auto;padding:34px 42px 54px}.connector-detail-hero{display:grid;grid-template-columns:auto minmax(0,1fr);gap:18px;align-items:center;padding-bottom:28px;border-bottom:1px solid var(--border-light,color-mix(in srgb,var(--text,var(--horizon-border)) 8%,transparent))}.connector-detail-hero h2{margin:0 0 7px;font-size:28px;letter-spacing:-.04em}.connector-detail-hero p{margin:0;color:var(--text-sub,var(--horizon-muted));font-size:13px;line-height:1.55}
    .connector-detail-section{padding:26px 0;border-bottom:1px solid var(--border-light,color-mix(in srgb,var(--text,var(--horizon-border)) 8%,transparent))}.connector-detail-section h3{margin:0 0 14px;font-size:14px}.connector-capabilities{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.connector-capability{padding:12px;border-radius:10px;background:var(--bg-input,var(--horizon-raised));color:var(--text-muted,var(--horizon-muted));font-size:11px;line-height:1.45}
    .connector-detail-actions{display:flex;flex-wrap:wrap;gap:9px;margin-top:24px}.connector-primary{height:38px;border:0;border-radius:10px;padding:0 15px;background:var(--text,var(--horizon-surface));color:var(--bg,var(--horizon-text));font:inherit;font-size:12px;font-weight:820;cursor:pointer}.connector-primary:disabled{opacity:.55;cursor:wait}
    .connector-request-dialog{position:absolute;inset:0;z-index:8;display:none;place-items:center;padding:20px;background:color-mix(in srgb, var(--horizon-surface) 55%, transparent);backdrop-filter:blur(10px)}.connector-request-dialog.open{display:grid}.connector-request-card{width:min(440px,100%);display:grid;gap:14px;padding:20px;border:1px solid var(--border,var(--horizon-border));border-radius:15px;background:var(--bg-surface,var(--horizon-surface));box-shadow:0 24px 80px rgba(0,0,0,.45)}.connector-request-card h3{margin:0;font-size:17px}.connector-request-card p{margin:0;color:var(--text-sub,var(--horizon-muted));font-size:12px;line-height:1.5}.connector-request-card input{height:42px;border:1px solid var(--border,var(--horizon-border));border-radius:10px;padding:0 12px;background:var(--bg-input,var(--horizon-raised));color:var(--text,var(--horizon-text));font:inherit;outline:0}.connector-request-actions{display:flex;justify-content:flex-end;gap:8px}
    @keyframes connector-marquee{from{transform:translateX(0)}to{transform:translateX(-50%)}}
    @media(max-width:900px){.connectors-panel{grid-template-columns:1fr;grid-template-rows:auto minmax(0,1fr);width:calc(100vw - 16px);height:calc(100dvh - 16px);border-radius:15px}.connectors-sidebar{gap:8px;padding:10px;border-right:0;border-bottom:1px solid var(--border-light,color-mix(in srgb,var(--text,var(--horizon-border)) 8%,transparent))}.connectors-nav{flex:none;flex-direction:row;overflow-x:auto}.connectors-nav-label,.connectors-sidebar-card{display:none}.connectors-filter{width:auto;min-width:max-content;min-height:36px}.connectors-grid{grid-template-columns:1fr}.connectors-intro{min-height:300px}.connectors-catalog{padding:24px 16px 40px}.connector-detail-body{padding:24px 18px 40px}.connector-capabilities{grid-template-columns:1fr}}
